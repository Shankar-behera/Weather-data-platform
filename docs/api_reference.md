...
Gets or creates Databricks SQL connection.
...
Executes SQL query on Databricks.
...
create_bronze_table()
Creates bronze table if it doesn't exist.

create_silver_table()
Creates silver table if it doesn't exist.

create_gold_table()
Creates gold table if it doesn't exist.

insert_bronze_records(records: List[RawWeatherRecord])
Inserts records into bronze table.
...
Inserts records into silver table.
...
Optimizes Delta table with Z-order and compaction.
...
Queries table at a specific point in time.
